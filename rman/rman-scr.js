    ...
    'http://47.24.20.103:5922/days',
     ...
     function(err, data) {
       if (err) {
        ...
        console.log('Post days: '+ JSON.stringify(wkarr));
        //var data=wkarr;
        //callback(null, data);
     }
    }
...
function makeWeek(){
  ...
  wkarr=[];
    for(i=0;i<7;i++){
      if(x[i].checked){
        wkarr.push(parseInt(x[i].value));
      }
    }
       console.log("inside of Make Week: "+ JSON.stringify(wkarr));
    /*console.log(wkarr);*/
        postDays();
}
     ...
     y[11]=document.getElementById("t2r4c13").innerHTML;
      ...
      for(i=0;i<12;i++){
        var object={pin:y[i]};
          ...
          console.log("inside manualMode: mnarr = "+ JSON.stringify(mnarr));
        /*console.log(mnarr);*/
          postManual(mnarr);
          getStatus();
}
